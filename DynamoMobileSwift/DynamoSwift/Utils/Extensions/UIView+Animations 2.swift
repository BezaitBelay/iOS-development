...
extension UIView {
    ...
    func startWiggle(cycleDuration: Double = 0.25, totalDuration: Double? = nil, displacement: CGFloat = 1.0, degreesRotation: CGFloat = 2.0) {
        ...
        if let totalDuration = totalDuration {
            delay(totalDuration) { [weak self] in
                self?.stopWiggle()
            }
        }
    }
